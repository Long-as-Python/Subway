...
namespace Battlehub.MeshDeformer2
{
    ...
    public class MeshDeformer : SplineBase
    {
        ...
        public void WrapAndDeform(ScaffoldWrapper scaffold, Mesh mesh = null)
        {
            if (scaffold.Obj != null)
            {
                Mesh meshToUse;

                // if(scaffold.Scaffold.AltMesh)
                //     meshToUse = scaffold.Scaffold.AltMesh;

                if (mesh != null)
                    meshToUse = mesh;
                else
                    meshToUse = m_original;
                ...
                if (filter != null)
                {
                    ...
                    filter.sharedMesh = Instantiate(meshToUse);
                    filter.sharedMesh.name = meshToUse.name + " Deformed";

                    scaffold.Wrap(filter.sharedMesh, colliderMesh, m_axis, scaffold.CurveIndices, m_sliceCount);
                    scaffold.Deform(this, meshToUse, m_colliderOriginal);
                    scaffold.RecalculateNormals();
                }
            }
        }
        ...
        public GameObject Append(Vector3 from, Vector3 to)
        {
            GameObject segment = CreateSegment();
            m_original.GetBounds(m_axis, out from, out to);
            ScaffoldWrapper previous = AppendSegment(segment, from, to);
